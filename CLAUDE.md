...
現状、地図表示機能(F-001)・エクスポート/デプロイスクリプト(F-004/F-005)は実装済み。レイヤー切替(F-002)は昼/夜/地形の3レイヤーまで実装済みで、バイオームレイヤーの追加(issue #17でMVPスコープに変更)は未実装。waypoint表示・編集機能(v1.1以降)は未実装。
